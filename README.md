...
The example above demonstrates the following style guidelines:
 ...
 + Don't add modifiers such as `internal` when they're already the default (unless you are writing a library or framework). 
...
Marking classes or members as `final` when possible for compiler optimizations. The use of `final` can sometimes clarify your intent and is worth the cost. In the below example, `Box` has a particular purpose and customization in a derived class is not intended. Marking it `final` makes that clear.
...
Developers should never create strong reference cycles. Analyze your object graph and prevent strong cycles with `weak` and `unowned` references. Alternatively, use value types (`struct`, `enum`) to prevent cycles altogether.
...
By default, applications and executables should use the implicit `internal` access control. Full access control annotation in apps can distract from and provide no extra value or clarity since an executable (e.g. `.app`) cannot exposed symbols to be linked against in other Swift modules.  Using `private` and `fileprivate` appropriately, however, adds clarity and promotes encapsulation. Prefer `private` to `fileprivate`; use `fileprivate` only when the compiler insists. 

Libraries, however should always declare their exposed API as `public` or  `open` if required for OOP patterns like abstract classes. Libraries should always use explicit access control and prefer `internal` over `private` for more inlining in the same Swift module.
